...
namespace IS_Proj_HIT.Controllers
{
    ...
    public class PCAController : Controller
    {
        ...
        [Authorize(Roles = "Administrator, Nursing Student, Nursing Faculty, HIT Faculty")]
        ...
        public IActionResult DeleteAssessment(int assessmentId, long encounterId)
        {
            ...
            if(exists){
                TempData["msg1"] = "The selected record was deleted.";
                var assessment = _repository.PcaRecords
                .Include(pca => pca.Encounter)
                .Include(pca => pca.Pcacomments)
                .ThenInclude(com => com.PcacommentType)
                ...
                ICollection<Pcacomment> comments = assessment.Pcacomments;

                foreach(Pcacomment comment in comments){
                    _repository.DeletePcaComment(comment);
                }
                
                _repository.DeletePcaRecord(assessment);
            }
            else{
                TempData["msg1"] = "The record does nto exist";
            }
